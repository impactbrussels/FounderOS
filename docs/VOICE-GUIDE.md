...
> so the rules stay identical across both.

## The voice in one line

**Proof before hype, in plain words.** Precise, evidence-first, anti-hype. Write like a good
advisor who respects the founder's time: direct, concrete, useful. No stories. No fluff. Earned
clarity, not pontificating.
...
Each skill does one founder job, produces a real artifact, and ends with a copy-paste prompt for
non-coders. Keep the body predictable and skimmable:

- **Lead with the job, not the preamble.** The founder should know in one line what this skill
  produces and when to run it.
- **Number the method.** Steps are a numbered list; each step is an action with a bold lead.
- **Name what needs a human call.** Every skill that touches an irreversible decision (equity,
  incorporation, a key hire) flags it and adds the not-legal/financial-advice caveat.
- **End with the artifact and the next step.** Say what the founder now has and which skill to run
  next.

Tables and short examples do real work; use a table where prose would ramble.
...
- **Anchor with specifics.** A real number, a named tool, a concrete object lowers the machine signal
  more than any adjective. Prefer "a 30-day runway" to "a short cash position."
...
- **No em-dashes or en-dashes in prose.** Commas, full stops, colons; or split the sentence. Unspaced
  en-dash for number ranges only (2026-2035). Headings use a full stop, no dash.
- **Bold** for key phrases and takeaways. *Italics* rarely. No ALL CAPS for emphasis.

## Evidence discipline (the anti-hype engine)

- Numbers are **exact**, never "significant." A claim must **survive a web check** or it does not
  ship. Contested figures carry a `[claimed in source]` tag and a source.
- Never state a guess as fact. Never fabricate a company, a figure, or a quote. Never invent market
  numbers or legal/tax thresholds; tell the founder to source or confirm them. A demo is not
